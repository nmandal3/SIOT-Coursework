...
## The SKINsense Physical Prototype Setup
...
3. Assemble the GSR sensor by plugging in the 2-pin grove cable end of the finger part into the mini circuit board 2-pin female connection. Use the other 4-wire grove cable provided - plug one end into the circuit board 4-pin female connection, and the other end into the A0 channel on the grove base HAT.
...
8. Test SSH connection by going to a terminal in any device on the hotspot network, then enter the following in the terminal, providing the password created during configuration in Step 5 when prompted:
```
...
* Install the Paho MQTT library - I used this [tutorial](https://bytebeam.io/blog/getting-started-with-mqtt-on-raspberry-pi-using-python/) by Bytebeam.
...
### Sensors:
* **GSR Sensor**: Measures skin dryness levels, using finger gloves with electrodes to make contact with skin. The electrical resistance between electrodes decreases with increased skin moisture, where a higher output value indicates dryer skin
* **HTU21D Sensor**: Measures temperature and humidity, and voltage output is a digital signal (therefore an ADC isn't required).
...
* `plots_overlaid_group.m` plots the normalised data with in three graphs with overlaying data groups, which can be seen in the `/Graphs Plotting Overlaid Data` folder:
